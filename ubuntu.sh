...
echo ""
echo "For more information please visit http://lnmp.jp.ai/"
echo "========================================================================="
cur_dir=$(pwd)
...
if [ "$isinstallphp53" = "n" ]; then
	if [ -s php-5.2.17.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/web/php/php-5.2.17.tar.gz
	fi
	if [ -s php-5.2.17-fpm-0.5.14.diff.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/web/phpfpm/php-5.2.17-fpm-0.5.14.diff.gz
	fi
else
	if [ -s php-5.3.28.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/web/php/php-5.3.28.tar.gz
	fi
fi
...
if [ -s memcache-3.0.6.tgz ]; then
  ...
  wget -c http://soft.jp.ai/web/memcache/memcache-3.0.6.tgz
fi
...
wget -c http://soft.jp.ai/web/pcre/pcre-8.12.tar.gz
fi
...
if [ -s nginx-1.6.0.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/web/nginx/nginx-1.6.0.tar.gz
fi
...
if [ "$isinstallmysql55" = "n" ]; then
	if [ -s mysql-5.1.73.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/datebase/mysql/mysql-5.1.73.tar.gz
	fi
	...
	if [ -s mysql-5.5.37.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/datebase/mysql/mysql-5.5.37.tar.gz
	fi
else 
	if [ -s mariadb-5.5.37.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/datebase/mariadb/mariadb-5.5.37.tar.gz
	fi
fi
...
if [ -s libiconv-1.14.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/web/libiconv/libiconv-1.14.tar.gz
fi
...
if [ -s libmcrypt-2.5.8.tar.gz ]; then
  ...
  wget -c  http://soft.jp.ai/web/libmcrypt/libmcrypt-2.5.8.tar.gz
fi
...
if [ "$isinstallphp53" = "n" ]; then
	if [ -s phpmyadmin-latest.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/datebase/phpmyadmin/phpmyadmin-latest.tar.gz
	fi
else
	if [ -s phpMyAdmin-lasest.tar.gz ]; then
	  ...
	  wget -c http://soft.jp.ai/datebase/phpmyadmin/phpMyAdmin-lasest.tar.gz
	fi
fi
...
if [ -s p.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/prober/p.tar.gz
fi
...
if [ -s autoconf-2.13.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/lib/autoconf/autoconf-2.13.tar.gz
fi
...
if [ -s libxml2-2.7.8.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/lib/libxml/libxml2-2.7.8.tar.gz
fi
...
if [ -s mysql-openssl.patch ]; then
  ...
  wget -c http://soft.jp.ai/lnmp/ext/mysql-openssl.patch
fi
...
if [ -s freetype-2.4.12.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/lib/freetype/freetype-2.4.12.tar.gz
fi
echo "============================check files=================================="
}
...
cd php-5.2.17/
wget -c http://soft.jp.ai/web/php/bug/php-5.2.17-max-input-vars.patch
...
cd ext/openssl/
wget -c http://soft.jp.ai/lnmp/ext/debian_patches_disable_SSLv2_for_openssl_1_0_0.patch
...
if [ `getconf WORD_BIT` = '32' ] && [ `getconf LONG_BIT` = '64' ] ; then
        wget -c http://soft.jp.ai/web/zend/ZendOptimizer-3.3.9-linux-glibc23-x86_64.tar.gz
        tar zxf ZendOptimizer-3.3.9-linux-glibc23-x86_64.tar.gz
	mkdir -p /usr/local/zend/
	cp ZendOptimizer-3.3.9-linux-glibc23-x86_64/data/5_2_x_comp/ZendOptimizer.so /usr/local/zend/
else
        wget -c http://soft.jp.ai/web/zend/ZendOptimizer-3.3.9-linux-glibc23-i386.tar.gz
...
wget -c http://soft.jp.ai/lnmp/ext/init.d.php-fpm5.2
...
if [ `getconf WORD_BIT` = '32' ] && [ `getconf LONG_BIT` = '64' ] ; then
	wget -c http://soft.jp.ai/web/zend/ZendGuardLoader-php-5.3-linux-glibc23-x86_64.tar.gz
	tar zxf ZendGuardLoader-php-5.3-linux-glibc23-x86_64.tar.gz
	mkdir -p /usr/local/zend/
	cp ZendGuardLoader-php-5.3-linux-glibc23-x86_64/php-5.3.x/ZendGuardLoader.so /usr/local/zend/
else
	wget -c http://soft.jp.ai/web/zend/ZendGuardLoader-php-5.3-linux-glibc23-i386.tar.gz
...
wget -c http://soft.jp.ai/lnmp/ext/init.d.nginx
...
echo "Please visit http://code.jp.ai/wiki/lnmp feedback errors and logs."
