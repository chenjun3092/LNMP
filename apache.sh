...
printf "\n"
printf "For more information please visit http://lnmp.jp.ai \n"
printf "=======================================================================\n"
cur_dir=$(pwd)
...
if [ -s httpd-2.2.27.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/web/apache/httpd-2.2.27.tar.gz
fi
...
if [ -s mod_rpaf-0.6.tar.gz ]; then
  ...
  wget -c http://soft.jp.ai/web/apache/rpaf/mod_rpaf-0.6.tar.gz
fi
...
sed -i 's/#ServerName www.example.com:80/ServerName lnmp.jp.ai:88/g' /usr/local/apache/conf/httpd.conf
sed -i 's/ServerAdmin you@example.com/ServerAdmin '$ServerAdmin'/g' /usr/local/apache/conf/httpd.conf
#sed -i 's/lnmp.jp.ai/'$domain'/g' /usr/local/apache/conf/extra/httpd-vhosts.conf
...
wget -c http://soft.jp.ai/lnmp/ext/init.d.httpd
